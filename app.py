                ...
                if int(cls) == label_index and confidences[i] > confidence_threshold:
                    ...
                    if ocr_result and isinstance(ocr_result, list) and len(ocr_result) > 0 and \
                        ...
                        draw = ImageDraw.Draw(pil_image)
                         # 获取文本宽度和高度
                        text_width, text_height = draw.textsize(license_plate_number.upper(), font=font)
                        
                        # 计算文本在图片正上方的位置
                        image_width, image_height = pil_image.size
                        text_x = (image_width - text_width) // 2
                        text_y = 10  # 固定距离图片顶部的位置
                        
                        draw.text((text_x, text_y), f"{license_plate_number.upper()}",
